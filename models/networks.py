...
def get_norm_layer(norm_type='instance', num_groups=1):
    if norm_type == 'batch':
        ...
        raise NotImplementedError('undefined [%s] ' % norm_type)
    return norm_layer
...
def get_norm_args(norm_layer, nfeats_list):
    if hasattr(norm_layer, '__name__') and norm_layer.__name__ == 'NoNorm':
        ...
        raise NotImplementedError('undefined [%s] ' % norm_layer.func.__name__)
    return norm_args
...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        ...
        return NotImplementedError('undefined [%s] ', opt.lr_policy)
    return scheduler
    ...
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            if init_type == 'normal':
                ...
                raise NotImplementedError('undefined [%s] ' % init_type)
...
def define_loss(opt):
    if opt.dataset_mode == 'classification':
        ...
        # funcion de perdida para la reconstruccion
...
class MeshConvNet(nn.Module):
    """
    Red para aprender una descripcion global de la figura (clasificacion)
    """
...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, unroll=0, residual=True,
                 batch_norm=True, transfer_data=True, final_layer = False):
        ...
        if fcs is not None:
            ...
            if global_pool is not None:
                if global_pool == 'max':
                    ...
                    assert False, 'undefined %s ' % global_pool
            else:
                last_length *= pools[-1]
...
class MeshDecoder(nn.Module):
    def __init__(self, unrolls, convs, blocks=0, batch_norm=True, transfer_data=True):
        ...
        self.final_conv = UpConv(convs[-2], convs[-1], blocks=blocks, unroll=False,
                                 batch_norm=batch_norm, transfer_data=False, final_layer=True)
