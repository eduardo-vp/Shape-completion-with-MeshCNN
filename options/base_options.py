...
class BaseOptions:
    def __init__(self):
        print("Base Options: __init__")
        self.parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        self.initialized = False

    def initialize(self):
        print("Base Options: initialize")
        # parametros de la data
        self.parser.add_argument('--dataroot', required=True, help='path to meshes (should have subfolders train, test)')
        self.parser.add_argument('--dataset_mode', choices={"classification", "segmentation", "completion"}, default='completion')
        # numero maximo de aristas en el input
        self.parser.add_argument('--ninput_edges', type=int, default=3080, help='# of input edges (will include dummy edges)')
    ...
    def parse(self):
        ...
        # cargar el self.opt los parametros
        self.opt, unknown = self.parser.parse_known_args()
        self.opt.is_train = self.is_train   # train or test

        # revisar los gpu que se colocaron en el .sh
        ...
        # si se va a entrenar, imprimir todas las opciones de entrenamiento 
        if self.is_train:
            print('------------ Options -------------')
            ...
            with open(file_name, 'wt') as opt_file:
                ...
                for k, v in sorted(args.items()):
                    opt_file.write('%s: %s\n' % (str(k), str(v)))
                opt_file.write('-------------- End ----------------\n')
        # devolver opt que dentro maneja todas las opciones y configuraciones establecidas
        return self.opt
